...
<div class="container py-4">
    ...
    <form action="{{ route('student.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            @if($article->status == 'rejected')
                                <select class="form-select" name="status">
                                    <option value="draft">Draft - Kirim untuk Review</option>
                                </select>
                                <small class="text-muted">Artikel yang ditolak hanya bisa disimpan sebagai draft untuk direview ulang</small>
                            @elseif($article->status == 'published')
                                <input type="hidden" name="status" value="published">
                                <div class="alert alert-success">
                                    <i class="bi bi-check-circle"></i> Artikel sudah dipublikasi
                                </div>
                            @else
                                <select class="form-select" name="status">
                                    <option value="draft">Draft - Kirim untuk Review</option>
                                </select>
                                <small class="text-muted">Artikel akan dikirim ke guru untuk direview</small>
                            @endif
                        </div>
